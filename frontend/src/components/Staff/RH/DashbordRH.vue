...
async function fetchStats() {
    const res = await fetch('http://localhost:8080/api/staff/rh/stats', {
...
async function fetchTasks() {
    const res = await fetch('http://localhost:8080/api/staff/rh/tasks', {
        headers: { 'Authorization': `Bearer ${token}` }
    });
    const json = await res.json();
  ...
  <div class="app-container">
    ...
    <aside class="fixed left-0 top-0 h-full w-[260px] bg-[#70cfcb] shadow-2xl flex flex-col h-full py-6 z-50">
      ...
      <nav class="flex-1 space-y-2">
        ...
        <a class="bg-white text-[#70cfcb] rounded-full mx-4 px-4 py-3 flex items-center gap-3 font-bold transition-all scale-95 active:scale-90" href="#">
    ...
    <main class="ml-[260px] pt-24 px-8 pb-12">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
        ...
        <div class="bg-white p-6 rounded-xl shadow-[0_15px_30px_-15px_rgba(112,207,203,0.04)] border border-slate-50 flex items-center gap-4">
          <div class="w-14 h-14 bg-[#70cfcb]/10 rounded-2xl flex items-center justify-center">
            <span class="material-symbols-outlined text-[#70cfcb] text-3xl" data-icon="groups">groups</span>
          </div>
          <div>
        ...
        <!-- Absences Justifiées -->
        <div class="bg-white p-6 rounded-xl shadow-[0_15px_30px_-15px_rgba(112,207,203,0.04)] border border-slate-50 flex items-center gap-4">
          <div class="w-14 h-14 bg-[#70cfcb]/20 rounded-2xl flex items-center justify-center">
            <span class="material-symbols-outlined text-[#006947] text-3xl" data-icon="verified_user">verified_user</span>
          </div>
          <div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-[0_15px_30px_-15px_rgba(112,207,203,0.04)] overflow-hidden">
          <div class="px-6 py-5 border-b border-slate-50 flex items-center justify-between">
            ...
            <span class="px-3 py-1 bg-[#70cfcb]/10 text-[#006947] text-label-md rounded-full">{{ absences.length }} absence(s)</span>
          </div>
          <div class="p-6">
            <table class="w-full text-left">
              ...
              <tbody class="text-body-sm">
                ...
                <tr v-for="absence in absences" :key="absence.id" class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td class="py-4">
                    <span :class="absence.is_justified ? 'bg-[#70cfcb]/30 text-[#006947]' : 'bg-[#ffdad6] text-[#ba1a1a]'"
                      class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold">
                      <span :class="absence.is_justified ? 'bg-[#006947]' : 'bg-[#ba1a1a]'" class="w-1.5 h-1.5 rounded-full"></span>
        ...
        <div class="space-y-5">
          <div class="bg-[#70cfcb] p-6 rounded-xl text-white shadow-lg relative overflow-hidden">
            <div class="relative z-10">
              <h3 class="font-headline-lg mb-1">Tâches du jour</h3>
              <p class="text-[#e0f7f5] text-body-sm mb-4">{{ tasks.length }} tâche(s) assignée(s)</p>
              <ul class="space-y-3">
                <li v-if="tasks.length === 0" class="text-[#e0f7f5] text-sm">Aucune tâche trouvée</li>
                <li v-for="task in tasks" :key="task.id"
                  ...
                  <span class="material-symbols-outlined text-[#e0f7f5]">
                    {{ task.is_completed ? 'check_circle' : 'task_alt' }}
                  </span>
                  <div>
                    <p class="text-label-md font-bold">{{ task.name }}</p>
                    <p class="text-[10px] text-[#e0f7f5] opacity-80">{{ task.description }}</p>
                  </div>
                </li>
              </ul>
            </div>
